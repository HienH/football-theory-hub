...
const ArticleCard = ({ article, featured = false }: ArticleCardProps) => {
  return (
    <Link
      ...
      className={`group block rounded-lg border border-border bg-background p-6 transition-all hover:border-gold/40 hover:shadow-md ${
        featured ? "md:col-span-2 bg-secondary/30" : ""
      }`}
    >
      {featured && (
        <span className="mb-3 inline-block rounded-full bg-gold-light border border-gold/20 px-3 py-1 text-xs font-semibold text-gold uppercase tracking-wider">
          Featured
        </span>
      )}
